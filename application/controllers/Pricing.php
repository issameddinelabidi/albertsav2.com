...
*/
            ...
            case 'checkout.session.completed':
                ...
                switch ($event->data->object['mode']) {
                    case 'setup':
                        ...
                        $priceId = 'price_1RrJpcLdvghffX5nYiC5yG3c'; //TEST

                        $this->nx_client = new NxClient([WS_URL, null, null, ['Authorization: Basic ' . ADMIN_BASIC]]);

                        $get_user = $this->nx_client->getList('User', ['filter' => ['stripe_id,eq,' . $customerId]]);

                        if (!empty($get_user[0]['id'])) {
                            //If member until dépassé, mettre today, sinon date prévue

                            $nb_annonces = $get_user[0]['max_annonces'];
                        } else {
                            throw new Exception("User not found with stripe id", 400005);
                        }
                        ...
                        try {
                            ...
                            \Stripe\Subscription::create([
                                ...
                                'items' => [[
                                    'price' => $priceId,
                                    'quantity' => $nb_annonces // volume facturé plus tard via usage_records
                                ]],
                                'default_payment_method' => $paymentMethodId,
    ...
    public function getStripeInvoices()
    {
        ...
        $stripe_second_account = new \Stripe\StripeClient(getenv('STRIPE_SK'));
